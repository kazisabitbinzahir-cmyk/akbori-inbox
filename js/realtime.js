...
function handleNewMessage(payload){
  ...
  if(!conv)return;
  if(msg.role==='agent'){var sendingLocal=conv.messages.find(function(m){return m.sending&&m.has_image===!!msg.has_image;});if(sendingLocal){sendingLocal.sending=false;sendingLocal.mid=msg.mid;sendingLocal.id=msg.id;if(msg.image_url)sendingLocal.image_url=msg.image_url;renderMsgs(conv.messages);if(selC&&selC.user_id===msg.user_id)initMsgScroll(conv);return;}}
